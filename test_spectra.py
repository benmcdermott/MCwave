...
def test_sort_spec():
    ...
    e = np.array([0.0,3.0,2.0,4.0,1.0])
    ...
    assert np.array_equal(esort,etest) , 'energy not sorted by wavenumber'


def test_bin_spec():
    k = np.array([0.0,0.1,0.2,1.0,1.1,1.2,2.0,2.1,2.2])
    e = np.linspace(1.0,9.0,9)

    test_spec = np.array([6.0,15.0,24.0])

    spec = spectra.bin_spec(k, e, 2*len(test_spec))

    print(spec)

    assert np.array_equal(spec,test_spec), 'spectrum binned incorrectly'
